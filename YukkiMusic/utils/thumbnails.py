...
async def get_qthumb(videoid):
    ...
    try:
        ...
        y=changeImageSize(900, 900,circle(youtube)) 
        background.paste(y,(400,550),mask=y)
        draw = ImageDraw.Draw(background)
        arial = ImageFont.truetype("Love/font2.ttf", 30)
        font = ImageFont.truetype("Love/font.ttf", 70)
        draw.text((1600, 600), f"Title: {title[:50]} .", (255, 255, 255), font=font)
        draw.text((1790, 990), f"Duration: {duration}", (255, 255, 255), font=font)
        draw.text((1790, 1190), f"Views: {views}", (255, 255, 255), font=font)
        try:
            os.remove(f"cache/thumb{videoid}.png")
